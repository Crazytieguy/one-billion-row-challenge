...
impl WeatherStation {
    const fn new(id: &'static str, mean_temperature: f64) -> Self {
        Self {
            id,
            mean_temperature,
        }
...
fn estimate_file_size(num_rows_to_create: usize) -> String {
    let total_name_bytes: usize = STATIONS.iter().map(|s| s.id.len()).sum();
    ...
    format_size(estimated_size as u64, humansize::BINARY)
}

#[allow(clippy::cast_possible_truncation)]
#[allow(clippy::cast_sign_loss)]
fn build_test_data(num_rows_to_create: usize, output_file: &PathBuf) -> Result<()> {
    ...
    for i in 0..num_rows_to_create {
        ...
        let temp = station.measurement(&mut rng);
        writeln!(writer, "{};{temp:.1}", station.id)
...
fn main() -> Result<()> {
    ...
    let estimated_file_size = estimate_file_size(args.num_records);
    eprintln!("Estimated file size is: {estimated_file_size}");

    build_test_data(args.num_records, &args.output_file)?;
